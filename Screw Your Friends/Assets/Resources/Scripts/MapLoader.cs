...
public class MapLoader : MonoBehaviour {
    ...
    [Header("offsets map location default=botleft")]
    ...
    private void buildMap() {
        ...
        this.transform.GetComponent<GameState>().MapDim(tex.height + yoffset);
        for (int x = 0; x < tex.width; x++) {
            for (int y = 0; y < tex.height; y++) {
                ...
                if (tmp.a != 0) {
                    ...
                    if (item != 999) {
                        GameObject obj = Instantiate(tiles[item], new Vector3(x + xoffset, y + yoffset, 0), Quaternion.identity) as GameObject;
                        if (item != (int)t.SPAWN)
